...
const blockW = 60, blockH = 40, cols = 100, rows = 100; // 10,000 Plots
cv.width = cols * blockW; cv.height = rows * blockH;

let scale = 0.08, pX = 0, pY = 0, pixels = {};
const imgCache = {};

function render() {
    ctx.fillStyle = "#fff"; ctx.fillRect(0, 0, cv.width, cv.height);
    ctx.strokeStyle = "#222"; ctx.lineWidth = 1.5; // Darker Grid
...
function searchPlot() {
    ...
    if (!val || id < 1 || id > 10000) return alert("Enter Valid ID: 1-10000");
...
db.ref('pixels').on('value', s => {
    ...
    document.getElementById('rem-count').innerText = 10000 - sold;
    render();
});

viewport.addEventListener('wheel', (e) => { e.preventDefault(); scale = Math.min(Math.max(scale * (e.deltaY > 0 ? 0.85 : 1.15), 0.03), 8); updateUI(); }, { passive: false });
